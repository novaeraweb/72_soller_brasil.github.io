<footer role="contentinfo">
	<div class="row">
		<div class="row-content buffer clear-after">
			<section id="top-footer">
				<div class="widget column four"><!-- la class="widget" è forse generata utomaticamente da wp -->
					<h4>Address</h4>
					<p>R. Dr. Napoleão Laureano, 707 <br> Vila Antartica, Botucatu/SP<br> 18608-590</p>
				</div>
				<div class="widget column three">
					<h4>Contact</h4>
					<p><strong>0800 777 57 58</strong><br>(14)3813-5758<br>sac@sollerbrasil.com.br</p>
					<p class="button">Be a reseller</p>
				</div>											
				<div class="widget meta-social column three">
					<h4>Follow Us</h4>
					<ul class="inline">
						<li><a href="#" class="facebook-share border-box"><i class="fa fa-facebook fa-lg"></i></a></li>
						<li><a href="#" class="instagram-share border-box"><i class="fa fa-instagram fa-lg"></i></a></li>
						<li><a href="#" class="linkedin-share border-box"><i class="fa fa-linkedin fa-lg"></i></a></li>
						<li><a href="#" class="youtube-share border-box"><i class="fa fa-youtube fa-lg"></i></a></li>
					</ul>
				</div>	
				<div class="widget column two">
					<h4>Languages</h4>
					<a href=""> <img src="img/icon-brazil.png"></a>
					<a href=""> <img src="img/icon-usa.png"></a>
					<a href=""> <img src="img/icon-spain.png"></a>
				</div>														
			</section><!-- top-footer -->
			<section id="bottom-footer">
				<p class="keep-left">S'ollér Brasil&copy; Beaty Matters | 2021 | All Rights Reserved.</p>
				<p class="keep-right"><a href="https://www.novaeraweb.com.br"> Desenvolvimento NovaEraWeb</a></p>
			</section><!-- bottom-footer -->			
		</div><!-- row-content -->	
	</div><!-- row -->
	<?php require_once "whatsapp.php" ?>
</footer>
<link href='https://fonts.googleapis.com/css?family=Montserrat:200,400,700|Open+Sans:400italic,700italic,200,400,700' rel='stylesheet' type='text/css'>