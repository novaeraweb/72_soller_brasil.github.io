...
$query_categorias = "SELECT c.*, i.ididioma FROM categoria c LEFT JOIN idioma i ON (i.ididioma = c.ididioma_cat) WHERE c.idcategoria = '$categoria_get' AND c.ididioma_cat = '3'";
	...
	<?php require_once "header.php" ?>
		<main role="main">
			...
			<div id="main">
				...
				<section class="row section" style="padding-top:7%;padding-bottom:7%;text-align: center;">
					<?php if ($categoria_get==25) { ?>
						<h2>Collections</h2>
					<?php } elseif  ($categoria_get==26) { ?>
						<h2>Coleções</h2>
					<?php } elseif  ($categoria_get==27) { ?>
						<h2>Colecciones</h2>
					<?php } ?>
				...
				<section class="row section section-volume">
					<!-- <div class="row-content buffer even clear-after">	 -->
						<div class="text-center">
							...
							<?php $query_professional = "SELECT * FROM produto WHERE idcolecao_prod = '$id' AND idlinha_prod = '6' AND ativo = 'Sim' ";
							...
							<?php // Carrega os produtos dinamicamente a partir das linhas (profissionais/manutenção) // A variável total verifica o nº de produtos de uma determinada coleção para decidir o tamanho da div
								  ...
								  foreach ($produtos as $produto) {?>
											...
											<div class="column seven texto" style="text-align:justify;">
												...
												<div class="peso">
													...
													if ($produto->peso_unico == '') { ?>
														<ul>
															<?php if ($produto->peso_p_br != NULL) { ?>
																<li><?=$produto->peso_p_br;?> | <?=$produto->peso_p_en;?> </li>
															<?php } ?>
															<?php if ($produto->peso_m_br != NULL) { ?>
															<li><?=$produto->peso_m_br;?> | <?=$produto->peso_m_en;?></li>
															<?php } ?>
															<?php if ($produto->peso_g_br != NULL) { ?>
										...
										<section class="row" style="padding: 5% 0 5% 0;"><hr>
										</section>
										<?php } else {?>
