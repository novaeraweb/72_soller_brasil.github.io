...
$hora = time();
    ...
    $sql = "UPDATE categoria SET arquivo = '$novo' WHERE idcategoria=$id";
    $Result = mysqli_query($soller, $sql) or die(mysqli_error($soller));
    
    $sql = "UPDATE categoria SET arquivo = '$novo' WHERE idcategoria=$id-1";
    $Result = mysqli_query($soller, $sql) or die(mysqli_error($soller));
...
header("Location: editar-imagens-categoria.php?id=$id");